...
class Scattershots extends Component
  ...
  scattershot: (targetOrPos) ->
    ...
    if @actions.move and @distance(@target or @getTargetPos(), true) > @attackRange
      @setAction 'move'
    else if @getCooldown('scattershot')
      @setAction 'attack'
    else
      @setAction 'scattershot'
      
    if @scattershottedOnce or @target?.health <= 0
      @scattershottedOnce = false
      @setAction 'idle'
      "done"
    else
      "scattershot"
  ...
  update: ->
    ...
    @scattershottedOnce = true if @plan
