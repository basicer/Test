...
class CastsSoulLink extends Component
  ...
  applySoulLink: ->
    ...
    @linkA.addEffect {name: 'fear', duration: 9001, reverts: true, setTo: true, targetProperty: 'soulLinked'}
    @linkB.addEffect {name: 'fear', duration: 9001, reverts: true, setTo: true, targetProperty: 'soulLinked'}
    
  linkedTakeDamage: (damage, attacker, momentum=null, fromSource=true) ->
    liveLinks = _.filter @linkTargets, (_l) -> return _l?.health > 0
    if fromSource
      linkTarget?.takeDamage (damage / liveLinks.length), attacker, momentum, false for linkTarget in @linkTargets
    else
